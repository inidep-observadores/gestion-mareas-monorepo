                    ...
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <h2 class="text-base font-black text-text uppercase tracking-tight leading-none">{{
                                observadorName }}</h2>
                            <p class="text-[10px] font-bold text-text-muted uppercase tracking-tighter mt-1">Historial
                                Operativo ({{ year }} - {{ year - 1 }})</p>
                        </div>
...
const loadHistorial = async () => {
    ...
    loading.value = true
    try {
        const [historialData, observadorData] = await Promise.all([
            observadoresApi.getHistorial(props.observadorId, props.year),
            observadoresApi.getObservador(props.observadorId)
        ])
        items.value = historialData
        observador.value = observadorData
    } catch (error) {
