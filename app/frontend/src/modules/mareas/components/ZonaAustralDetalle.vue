<template>
  <div v-if="data" class="mt-4 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
    ...
    <div v-if="showDates" class="flex flex-wrap gap-2 p-3 bg-surface border border-border rounded-xl max-h-40 overflow-y-auto custom-scrollbar">
      ...
      <p v-if="data.diasDetectadosMarea.length === 0" class="text-xs text-text-muted italic">No hay fechas detectadas.</p>
    </div>

    <!-- Desglose por Etapas -->
    <div class="space-y-3">
      <h5 class="text-[10px] font-black uppercase tracking-[0.2em] text-text-muted/60 px-1">Desglose por Etapa de Navegación</h5>
      <div v-for="etapa in data.etapas" :key="etapa.etapaId" 
        ...
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center gap-2">
            <span class="w-6 h-6 flex items-center justify-center bg-surface-muted rounded-lg text-xs font-bold text-text-muted border border-border">
              {{ etapa.nroEtapa }}
            </span>
            <span class="text-sm font-bold text-text">Etapa {{ etapa.nroEtapa }}</span>
          </div>
          <div class="flex items-center gap-1.5 px-2 py-0.5 bg-success/10 text-success rounded-full border border-success/20">
        ...
        <p v-else class="text-[10px] text-text-muted/50 italic mt-2">Sin actividad detectada en Zona Austral para esta etapa.</p>
      </div>
    </div>
  </div>
</template>
