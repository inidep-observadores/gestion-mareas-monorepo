    ...
    <div class="hidden lg:grid lg:grid-cols-12 lg:gap-8 min-h-[600px]">
      ...
      <div class="lg:col-span-4 flex flex-col gap-4">
        ...
        <div v-else class="flex-1 overflow-y-auto custom-scrollbar bg-surface rounded-2xl border border-border shadow-sm">
          <template v-for="(lote, index) in lotes" :key="lote.id">
          ...
          <div v-else-if="detalles" class="flex-1 overflow-y-auto custom-scrollbar bg-linear-to-b from-surface-muted/30 to-surface rounded-3xl border border-border flex flex-col h-full shadow-sm animate-fade-in">
              ...
              <div class="flex flex-col xl:flex-row justify-between items-start gap-8">
                <div class="min-w-0 flex-1">
                    ...
                    <h2 class="text-2xl font-black text-text leading-tight tracking-tight uppercase">
                       Lote de Protocolización #{{ detalles.items.length }}
                    </h2>
            ...
            <div class="p-8">
              ...
              <div class="grid grid-cols-1 gap-4">
                <div v-for="item in detalles.items" :key="item.id" class="group flex flex-col bg-surface border border-border border-l-4 border-l-primary p-4 rounded-xl shadow-sm hover:shadow-md transition-all">
                  <div class="flex items-center justify-between">
          ...
          <div v-else class="flex flex-col gap-3">
            ...
            >
              <div class="flex flex-col gap-1">
                <span class="font-black text-text uppercase tracking-widest text-[10px] leading-tight group-hover:text-primary transition-colors">{{ lote.usuario?.fullName || 'Sistema' }}</span>
                <div class="flex items-center gap-2 mt-1">
                  <Badge variant="light" size="sm" class="text-[8px] font-black uppercase px-2 tracking-tighter">{{ lote.canal }}</Badge>
                  <span class="text-[8px] font-bold text-text-muted uppercase italic">{{ lote._count?.items }} {{ lote._count?.items === 1 ? 'marea' : 'mareas' }}</span>
                </div>
              </div>
        ...
        <div v-else class="flex flex-col gap-4">
          ...
          <div v-else-if="detalles" class="bg-surface border border-border rounded-3xl shadow-xl overflow-hidden flex flex-col animate-scale-in">
             <div class="p-6 border-b border-border bg-linear-to-b from-surface-muted/50 to-surface">
                ...
                <h4 class="text-xl font-black text-text tracking-tighter leading-tight uppercase">Protocolización</h4>
                <div class="flex flex-col gap-0.5 mt-1">
                  <p class="text-[9px] font-bold text-text-muted uppercase tracking-tight">{{ formatDateGroup(detalles.fechaEnvio) }}</p>
                  <p class="text-[9px] font-black text-primary uppercase italic opacity-80">Enviado por {{ detalles.usuario?.fullName || 'Sistema' }}</p>
                </div>
             </div>

             <div class="p-4 flex flex-col gap-3">
                <div v-for="item in detalles.items" :key="item.id" class="bg-surface-muted/30 p-4 rounded-xl border border-border border-l-4 border-l-primary flex flex-col">
...
const selectLote = async (id: string) => {
  selectedLoteId.value = id
  loadingDetail.value = true
  detalles.value = null

  try {
    detalles.value = await mareasService.getProtocolizacionLoteDetalle(id)
  } catch (error) {
...
const shouldShowDateHeader = (lote: any, index: number) => {
  ...
  const prevLote = props.lotes[index - 1]
