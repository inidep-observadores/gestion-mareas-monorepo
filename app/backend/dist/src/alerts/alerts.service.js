...
let AlertsService = AlertsService_1 = class AlertsService {
    ...
    async create(createAlertDto, user) {
        ...
        if (existing) {
            if ([alerts_enums_1.AlertaEstado.DESCARTADA, alerts_enums_1.AlertaEstado.RESUELTA].includes(existing.estado)) {
                this.logger.log(`Alerta ${codigoUnico} ignorada (está ${existing.estado})`);
                return existing;
            }
            if ([alerts_enums_1.AlertaEstado.PENDIENTE, alerts_enums_1.AlertaEstado.SEGUIMIENTO].includes(existing.estado)) {
                return existing;
            }
    ...
    async update(id, updateAlertDto, user) {
        ...
        if (data.estado && [alerts_enums_1.AlertaEstado.SEGUIMIENTO, alerts_enums_1.AlertaEstado.DESCARTADA, alerts_enums_1.AlertaEstado.RESUELTA].includes(data.estado) && !comment?.trim()) {
