...
async function validatePna2026() {
    console.log('--- Iniciando Validación de Mareas vs PNA (Año 2026) ---');
    ...
    // 1. Obtener todas las mareas del 2026 con sus etapas y puertos
    try {
    ...
    const fileName = `validacion_pna_2026_${DateTime.now().toFormat('yyyyMMdd_HHmm')}.csv`;
...
async function validateEvent(prisma: any, marea: any, etapa: any, type: string, systemDate: Date, systemPort: any, tz: string) {
    ...
    // Normalización de identificadores para evitar problemas con ceros a la izquierda e inconsistencias de subcadenas
