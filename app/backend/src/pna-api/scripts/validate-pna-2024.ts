...
async function validatePna2024() {
    console.log('--- Iniciando Validación de Mareas vs PNA (Año 2024) ---');
    ...
    let mareas: any[] = []; // Declare mareas outside the try block

    // 1. Obtener todas las mareas del 2024 con sus etapas y puertos
    try {
    ...
    const fileName = `validacion_pna_2024_${DateTime.now().toFormat('yyyyMMdd_HHmm')}.csv`;
...
async function validateEvent(prisma: any, marea: any, etapa: any, type: string, systemDate: Date, systemPort: any, tz: string) {
    ...
    // Prioridad de búsqueda por MBPC si lo tenemos
    ...
    const pnaMatch = await prisma.pnaZarpadaArribo.findFirst({
        ...
        orderBy: {
            // El más cercano a la fecha del sistema
            fecha: 'asc'
        }
    });
    ...
    // Validar puerto
    // PNA tiene nombre de costera. El puerto del sistema tiene nombre y códigos.
    let portStatus = 'MATCH';
    const pnaPortName = pnaMatch.nombreCostera;
    const systemPortName = systemPort?.nombre || 'S/D';

    // Comparación simple por nombre (frecuente en histórico)
    // TODO: Mejorar con tabla de equivalencias si es necesario
    ...
    // Decidir estatus final
    let status = 'MATCH';

    // Comparación por día calendario (ignorar horas)
