/*
  ...
  - The `tipo_marea` column on the `mareas` table would be dropped and recreated. This will lead to data loss if there is data in the column.
  - Changed the type of `tipo_etapa` on the `mareas_etapas` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.

*/
-- 1. Drop Dependencies
DROP VIEW IF EXISTS vw_mareas_completas;
DROP INDEX IF EXISTS "identificadorMarea";

-- 2. Create Enums (Idempotent for deploy)
DO $$ BEGIN
    CREATE TYPE "TipoMarea" AS ENUM ('MC', 'CI');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
    CREATE TYPE "TipoEtapa" AS ENUM ('MC', 'CI');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
-- 3. Alter Tables
-- Mareas
ALTER TABLE "mareas" ALTER COLUMN "tipo_marea" DROP DEFAULT;
ALTER TABLE "mareas" ALTER COLUMN "tipo_marea" TYPE "TipoMarea" USING "tipo_marea"::"TipoMarea";
ALTER TABLE "mareas" ALTER COLUMN "tipo_marea" SET DEFAULT 'MC'::"TipoMarea";

-- Mareas Etapas
ALTER TABLE "mareas_etapas" ALTER COLUMN "tipo_etapa" TYPE "TipoEtapa" USING "tipo_etapa"::"TipoEtapa";
...
CREATE UNIQUE INDEX "identificadorMarea" ON "mareas"("anio_marea", "nro_marea", "tipo_marea");
