...
function parseEtapaDates(etapaStr) {
    if (!etapaStr)
        return null;
...
async function generateJsonl() {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        const diasEstimadosStr = row[5]?.trim();
        const diasZonaAustralStr = row[12]?.trim();
...
generateJsonl();
//# sourceMappingURL=generate-mareas-jsonl.js.map