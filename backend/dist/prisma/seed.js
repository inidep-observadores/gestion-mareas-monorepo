...
const normalize = (str, sortWords = false) => {
    if (!str)
        return '';
...
function getObserverContractsFromCsv() {
...
async function seedMareasFromJsonl() {
    ...
    const mapEstado = (estadoRaw) => {
        ...
        if (est.includes('navegando') || est.includes('ejecucion'))
            return estados.find((e) => e.codigo === 'EN_EJECUCION');
        if (est.includes('espera') || est.includes('designada'))
            return estados.find((e) => e.codigo === 'DESIGNADA');
        if (est.includes('cancelada') || est.includes('sinpesca'))
            return estados.find((e) => e.codigo === 'CANCELADA');
        return estados.find((e) => e.codigo === 'DESIGNADA');
    };
    console.log(`Procesando ${records.length} mareas desde JSONL...`);
    for (const data of records) {
        const { nroMarea, buqueName, obsName, especieStr, estadoStr, zarpadaEstimada, fechaZarpadaEstimada, empresa, etapas = [], diasEstimados, anioMarea = 2025, } = data;
...
async function main() {
    ...
    const getEstadoId = (codigo) => allEstados.find(e => e.codigo === codigo)?.id;
    ...
    if (fs.existsSync(jsonlPath)) {
        ...
        const refinedObservadores = observadoresData.map((obs) => {
        ...
        for (const obs of refinedObservadores) {
            ...
            const preferNew = (!existing.activo && obs.activo) ||
        ...
        if (duplicatedCodes.size > 0) {
            console.warn(`Se detectaron codigos internos duplicados (${[...duplicatedCodes].join(', ')}). ` +
    ...
    if (fs.existsSync(buquesJsonlPath)) {
        ...
        const getPesqueriaId = (flotaCodigo) => {
            return null;
        };
        const getPuertoId = (localidad) => {
            const loc = (localidad || '').toLowerCase().trim();
            if (!loc)
                return null;
            if (loc.includes('buenos aires'))
                return puertos.find(p => p.nombre.toLowerCase().includes('buenos aires'))?.id;
            if (loc.includes('madryn'))
                return puertos.find(p => p.nombre.toLowerCase().includes('madryn'))?.id;
            if (loc.includes('deseado'))
                return puertos.find(p => p.nombre.toLowerCase().includes('deseado'))?.id;
            if (loc.includes('san julian') || loc.includes('san julián'))
                return puertos.find(p => p.nombre.toLowerCase().includes('san julian'))?.id;
            if (loc.includes('rawson') || loc.includes('trelew'))
                return puertos.find(p => p.nombre.toLowerCase().includes('rawson'))?.id;
            if (loc.includes('comodoro') || loc.includes('rivadavia'))
                return puertos.find(p => p.nombre.toLowerCase().includes('comod'))?.id;
            if (loc.includes('ushuaia') || loc.includes('punta arenas'))
                return puertos.find(p => p.nombre.toLowerCase().includes('ushuaia'))?.id;
