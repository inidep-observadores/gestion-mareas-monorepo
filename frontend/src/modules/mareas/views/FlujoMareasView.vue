<template>
  <AdminLayout
    ...
    description="Seguimiento detallado de mareas por estado del proceso."
  >
    <div class="relative min-h-[calc(100vh-100px)] z-1">
      ...
      <div class="flex flex-wrap items-center gap-2 mb-6">
        ...
        <StatusFilterChip
          ...
          :value="kpi.value"
          :icon="kpi.icon"
          :active="!collapsedGroups.has(kpi.codigo)"
          :color-class="kpi.color"
          :bg-class="kpi.bg"
          :border-class="kpi.border"
          @click="toggleGroupCollapse(kpi.codigo)"
          ...
          >
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar">
              ...
              <template v-else-if="hasMareas">
                  ...
                  >
                    ...
                    >
                        <div class="flex items-center gap-2">
                            <span class="text-xs font-black uppercase tracking-widest text-text-muted">{{ group.label }}</span>
                            <span class="px-2 py-0.5 bg-surface-muted rounded-full text-[10px] font-bold text-text">{{ group.items.length }}</span>
                        </div>
                        <ChevronDownIcon 
                    ...
                    <div v-show="group.expanded" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      ...
                      >
                        ...
                        <div class="grid grid-cols-2 gap-2 mb-3 bg-surface-muted/30 p-2 rounded-xl border border-border/50">
                            <div>
                                <span class="text-[8px] font-black text-text-muted uppercase tracking-widest block mb-0.5">Zarpada</span>
                                <span class="text-[10px] font-bold text-text">{{ formatDate(marea.fecha_zarpada) }}</span>
                            </div>
                            <div>
                                <span class="text-[8px] font-black text-text-muted uppercase tracking-widest block mb-0.5">Últ. Arribo</span>
                                <span v-if="marea.fecha_arribo" class="text-[10px] font-bold text-text">{{ formatDate(marea.fecha_arribo) }}</span>
                                <span v-else-if="marea.estado_codigo === 'EN_EJECUCION'" class="text-[10px] font-black text-primary italic">Navegando...</span>
                ...
                <div class="hidden xl:block overflow-x-auto">
                  <table class="w-full text-left border-collapse">
                    <thead class="bg-surface-muted/50 text-[10px] font-black uppercase tracking-widest text-text-muted border-b border-border sticky top-0 z-10 backdrop-blur-sm">
                      <tr>
                        <th @click="toggleSort('id_marea')" class="px-4 py-3 w-28 cursor-pointer hover:text-primary transition-colors group">
                           <div class="flex items-center gap-1">
                             Marea
                        ...
                        <th @click="toggleSort('buque_nombre')" class="px-5 py-3 cursor-pointer hover:text-primary transition-colors group">
                            <div class="flex items-center gap-1">
                                Buque
                        ...
                        <th @click="toggleSort('fecha_zarpada')" class="px-5 py-3 cursor-pointer hover:text-primary transition-colors group">
                            <div class="flex items-center gap-1">
                                Zarpada
                        ...
                        <th @click="toggleSort('fecha_arribo')" class="px-5 py-3 cursor-pointer hover:text-primary transition-colors group">
                            <div class="flex items-center gap-1">
                                Último Arribo
                                <ChevronDownIcon v-if="sortBy === 'fecha_arribo'" class="w-3 h-3 text-primary transition-transform duration-300" :class="{ 'rotate-180': sortOrder === 'asc' }" />
                            </div>
                        </th>
                        <th v-if="!selectedMarea" @click="toggleSort('progreso')" class="px-5 py-3 cursor-pointer hover:text-primary transition-colors group">
                            <div class="flex items-center gap-1">
                                Progreso
                        ...
                        <th v-if="!selectedMarea" @click="toggleSort('alertas')" class="px-5 py-3 cursor-pointer hover:text-primary transition-colors group">
                            <div class="flex items-center gap-1">
                                Alertas
                    ...
                    >
                        <!-- Group Header Row -->
                        <tr 
                            class="bg-surface-muted/20 hover:bg-surface-muted/40 cursor-pointer transition-colors"
                            @click="toggleGroupCollapse(group.code)"
                        >
                            <td :colspan="selectedMarea ? 4 : 6" class="px-4 py-2">
                                <div class="flex items-center gap-3">
                                    <button class="w-6 h-6 flex items-center justify-center rounded-full bg-surface border border-border text-text-muted hover:text-primary transition-colors">
                                         <ChevronDownIcon 
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-xs font-black uppercase text-text tracking-wide">{{ group.label }}</span>
                                        <span class="px-2 py-0.5 bg-surface text-text-muted border border-border rounded-full text-[10px] font-bold">{{ group.items.length }}</span>
                                    </div>
                                </div>
                            </td>
                        </tr>
                        ...
                        >
                            ...
                            <td class="px-5 py-1.5">
                                <div class="flex flex-col" v-if="marea.fecha_arribo">
                                    ...
                                    <span class="text-[10px] text-text-muted leading-none mt-1">{{ marea.puerto_arribo || 'N/D' }}</span>
                                </div>
                                <div v-else-if="marea.estado_codigo === 'EN_EJECUCION'" class="flex items-center gap-1">
                                    <div class="w-1.5 h-1.5 rounded-full bg-primary animate-pulse"></div>
                                    <span class="text-[10px] font-black text-primary uppercase tracking-tighter italic">Navegando...</span>
                                </div>
              ...
              <div v-else class="p-20 flex flex-col items-center justify-center text-center">
                ...
                <p class="text-text-muted text-sm mt-1 max-w-xs">No se encontraron operaciones en este año operativo.</p>
              </div>
            </div>
          </div>
        </div>
