<template>
  <HudCard customClass="w-[210px]">
    <div class="p-3 border-b border-border/10 flex flex-col items-center gap-0 bg-surface/5 group/card cursor-pointer"
    ...
    <div v-show="!isCollapsed" class="p-2 flex flex-col gap-2 overflow-y-auto max-h-[35vh] custom-scrollbar">
      <div v-for="(stage, index) in stages" :key="index" @click="$emit('select-stage', stage)"
        ...
        <div class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-r-full opacity-60"
          :style="{ backgroundColor: stage.color || 'var(--color-primary)' }"></div>
        ...
        <div class="flex items-center justify-between ml-1">
          ...
          <span
            class="font-black px-2 py-0.5 rounded-lg bg-primary/10 text-primary border border-primary/20 shadow-[0_0_8px_rgba(var(--color-primary-rgb),0.1)] text-[10px]">
        ...
        <div class="ml-1 flex flex-col gap-0.5 opacity-80 text-[10px]">
          <div class="flex items-center gap-2 group/date cursor-pointer hover:text-primary transition-colors"
            @click.stop="$emit('select-date', stage.startDate, false)">
            ...
            <span class="text-text tabular-nums group-hover/date:underline decoration-primary/30 underline-offset-2">{{
              formatDate(stage.startDate) }}</span>
          </div>
          <div class="flex items-center gap-2 group/date cursor-pointer hover:text-primary transition-colors"
            @click.stop="$emit('select-date', stage.endDate || new Date().toISOString(), true)">
...
const calculateDuration = (start: string, end: string | null) => {
  ...
  // Normalizar a medianoche para cálculo inclusivo (16 al 19 = 4 días)
  d1.setHours(0, 0, 0, 0)
  d2.setHours(0, 0, 0, 0)
