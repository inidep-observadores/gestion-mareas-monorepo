<template>
  <HudCard customClass="w-[170px]">
    <div class="p-2 border-b border-border/10 flex flex-col items-center gap-0 bg-surface/5 group/card cursor-pointer" @click="isCollapsed = !isCollapsed">
      <div class="flex items-center justify-between w-full">
        <div class="flex flex-col">
          <h3 class="font-black uppercase tracking-[0.2em] text-text text-[8px]">
            {{ stages.length }} ETAPAS
          </h3>
          <p v-if="!isCollapsed" class="font-bold text-text-muted/60 uppercase tracking-tighter text-[7px]">
    ...
    <!-- Scrollable Area -->
    <div v-show="!isCollapsed" class="p-1.5 flex flex-col gap-1.5 overflow-y-auto max-h-[35vh] custom-scrollbar">
      <div
        v-for="(stage, index) in stages"
        :key="index"
        @click="$emit('select-stage', stage)"
        class="group relative flex flex-col gap-1 p-2 rounded-xl bg-surface/5 border border-border/5 hover:border-primary/30 transition-all cursor-pointer"
      >
        ...
        <div class="flex items-center justify-between ml-1">
          <span class="font-black uppercase tracking-tighter text-[9px]" :style="{ color: stage.color }">
            Etapa {{ index + 1 }}
          </span>
          <span class="font-black px-1.5 py-0.5 rounded-lg bg-primary/10 text-primary border border-primary/20 shadow-[0_0_8px_rgba(var(--color-primary-rgb),0.1)] text-[8px]">
            {{ stage.durationDays }}d
          </span>
        </div>

        <div class="ml-1 flex flex-col gap-0 opacity-80 text-[8px]">
