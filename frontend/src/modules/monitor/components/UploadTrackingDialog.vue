<template>
  <BaseModal :show="show" title="Importar Seguimiento" maxWidth="xl" @close="closeIfPossible">
    <template #title>
      <div class="flex flex-col">
        <h2 class="text-xs font-black text-text uppercase tracking-widest">Importar Seguimiento</h2>
        <p class="text-[9px] text-text-muted font-bold uppercase tracking-tight">Procesamiento de Archivos CSV de SIOP
        </p>
      </div>
    </template>
    ...
    <div class="space-y-6">
      <!-- Dropzone -->
      <div v-if="files.length === 0" @click="triggerFileInput" @dragover.prevent @drop.prevent="handleDrop"
        ...
        <div
          class="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
        ...
        <div class="text-center">
          ...
          <p class="text-[9px] text-text-muted uppercase font-bold mt-1">Suelte archivos CSV aquí o haga clic para
            buscar
          </p>
        </div>
      ...
      <div v-else class="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
        <div v-for="(f, idx) in files" :key="idx"
          ...
          <div class="flex items-center gap-4">
            <div
              class="w-10 h-10 rounded-xl bg-surface flex items-center justify-center shadow-theme-xs border border-border/5">
              <svg v-if="f.status === 'pending'" class="w-5 h-5 text-text-muted/40" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2">
                <path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z" />
                <polyline points="13 2 13 9 20 9" />
              </svg>
              <div v-if="f.status === 'processing'"
                class="w-5 h-5 border-2 border-primary border-t-transparent animate-spin rounded-full"></div>
              <svg v-if="f.status === 'success'" class="w-5 h-5 text-success" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="3">
                <polyline points="20 6 9 17 4 12" />
              </svg>
              <svg v-if="f.status === 'error'" class="w-5 h-5 text-error" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="3">
          ...
          <button v-if="f.status === 'pending' && !isUploading" @click="removeFile(idx)"
      ...
      <div v-if="files.length > 0" class="pt-6 border-t border-border/10 flex justify-between items-center">
        <Button v-if="!isUploading" variant="ghost" size="xs"
        ...
        <div class="flex gap-3">
          <Button v-if="!allFinished" variant="primary" size="sm" className="!rounded-xl shadow-lg shadow-primary/20"
            :disabled="isUploading" @click="startUpload">
            <span class="text-[10px] font-black uppercase tracking-widest">
              {{ isUploading ? 'Cargando...' : `Iniciar Carga (${files.length})` }}
            </span>
          </Button>
          <Button v-else variant="outline" size="sm" className="!rounded-xl" @click="$emit('close'); $emit('refresh')">
